...
public:
    Technique();
    ~Technique();
    virtual bool Init();         // инициализация через вызов функции Init().
    void Enable();

protected:
    bool AddShader(GLenum ShaderType, const char* pShaderText);   // обычный порядок
    bool Finalize();                                              //  Finalize() вызывается для линковки объектов. (Передаётся указатель на объект )
...
class LightingTechnique : public Technique     // LightingTechnique - представляет собой инвентарь света, используя основой функционал компиляции и линковки
{
public:
    LightingTechnique();
    virtual bool Init();       // Функция Init  вызывает Technique::AddShader() и Techique::Finalize() для генерации программы GLSL.
...
void GLUTBackendInit(int argc, char** argv);  // "GLUT backend", упрощает инициализацию GLUT'а

bool GLUTBackendCreateWindow(unsigned int Width, unsigned int Height, unsigned int bpp, bool isFullScreen, const char* pTitle);

void GLUTBackendRun(ICallbacks* pCallbacks);   // интерфейс ICallbacks, помогает в регистрации функций обратного вызова.
...
class Main : public ICallbacks           // Это скелет главного класса приложения,который отвечает за эффекты, загрузку текстуры и создание буферов вершин и индексов.
{
public:
    ...
    Main()
    {
        ...
        m_directionalLight.AmbientIntensity = 1.3f;  // Отвечает за освещение 
    }
    ...
    void Run()       // Run() вызывает GLUTBackendRun() и передает себя в качестве параметра.
    {
        GLUTBackendRun(this);
    }
